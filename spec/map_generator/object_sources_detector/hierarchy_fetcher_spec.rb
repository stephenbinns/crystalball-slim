...
describe Crystalball::MapGenerator::ObjectSourcesDetector::HierarchyFetcher do
  ...
  describe "#ancestors_for" do
    ...
    let(:constant) do
      double(ancestors: [ancestor1, ancestor3], singleton_class: double(ancestors: [ancestor2, ancestor3]))
    end
    ...
    it "provides all ancestors" do
      expect(subject).to contain_exactly(ancestor1, ancestor2, ancestor3)
    end
    ...
    context "when stop classes passed" do
      ...
      it "filters ancestors" do
        expect(subject).to contain_exactly(ancestor1, ancestor2)
      end
    end
  end
end

...
describe Crystalball::MapStorage::YAMLStorage do
  ...
  describe ".load" do
    ...
    it "loads yaml metadata and example_groups from file if it exists" do
      ...
      allow(path).to receive(:read).with(no_args).and_return({ commit: "123",
                                                               type: "Crystalball::ExecutionMap" }.to_yaml + { "UID1" => %w[
                                                                 1 2 3
                                                               ] }.to_yaml + { "UID100" => %w[a b c] }.to_yaml)
    ...
    context "when path is a directory" do
      ...
      context "when metadata info is inconsistent" do
        ...
        specify do
          expect do
            subject
          end.to raise_error("Can't load execution maps with different metadata. Metadata: [{commit: \"123\", type: \"Crystalball::ExecutionMap\"}, {commit: \"456\", type: \"Crystalball::ExecutionMap\"}]")
        end
      end
    end
